...
# 反向索引：同义词 → 标准名
SYNONYM_DICT_DERMNET_REVERSE = {}
for std, synonyms in SYNONYM_DICT_HAM10000.items():
    for w in synonyms:
        SYNONYM_DICT_DERMNET_REVERSE[w] = std


def pred_to_class_23(pred_words) -> int:
    """
    ...
    # 1. 预测词直接命中
    if pred_str in SYNONYM_DICT_DERMNET_REVERSE:
        return list(SYNONYM_DICT_HAM10000.keys()).index(SYNONYM_DICT_DERMNET_REVERSE[pred_str])
...
def word_hit_metrics(pred_file: str,
    ...
    for f in filenames:
        ...
        y_pred_id.append(pred_to_class_23(str(raw_pred)))
...
if __name__ == "__main__":
    ...
    # print(metrics)
    scores = word_hit_metrics('/Volumes/T7/SkinGPT-X-EvaluationResults/Dermnet/Hulumed/diagnosis_results.csv',
                              '/Volumes/T7/SkinGPT-X-EvaluationResults/Dermnet/Hulumed/diagnosis_results.csv',
                              img_dir=BASE_IMAGE_DIRECTORY,
                              out_csv='/Volumes/T7/SkinGPT-X-EvaluationResults/Dermnet/Hulumed/metrics_results.csv')
    print(f"{scores}")
