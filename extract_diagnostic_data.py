...
def main():
    # 文件路径
    base_path = "/Volumes/T7/Skingpt_X/SkinGPT-X-EvaluationResults/Dermnet/test"
    ...
    # 获取所有图像文件名（从第一个文件获取）
    rag_data = load_json_file(files['RAG_output'])
    ...
    for image_name in image_names:
        row_data = {
            ...
            'RAG_Diagnostic_Assessment': '',
            'SkinGPT_Diagnostic_Assessment': '',
            'WebSearch_Diagnostic_Assessment': '',
            'CaseReview_PrimaryDiagnosis': '',
            'Reasoning_PrimaryDiagnosis': '',
            'TreatmentRecommend_PrimaryDiagnosis': ''
        }
    ...
    # 保存CSV文件
    output_file = "/Volumes/T7/Skingpt_X/diagnostic_data_extraction.csv"
    
    # 定义列的顺序
    fieldnames = [
        'image_name', 'label',
        'RAG_Diagnostic_Assessment', 'SkinGPT_Diagnostic_Assessment', 'WebSearch_Diagnostic_Assessment',
        'CaseReview_PrimaryDiagnosis', 'Reasoning_PrimaryDiagnosis', 'TreatmentRecommend_PrimaryDiagnosis'
    ]
